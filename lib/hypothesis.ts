...
export interface KendallTauResult {
  tau: number;
  pValue: number;
  significant: boolean;
  direction: "accelerating" | "decelerating" | "none";
  n: number;
}

/**
 * Kendall's tau-b rank correlation between time index and velocity changes.
 * Tests whether improvement rates are monotonically increasing (acceleration)
 * or decreasing (deceleration). More robust than t-test for irregular spacing.
 * Used by the independent adversarial review (combined deceleration p=0.0057).
 */
export function kendallTauTest(
  times: number[],
  scores: number[]
): KendallTauResult {
  if (times.length < 4) {
    return { tau: 0, pValue: 1, significant: false, direction: "none", n: 0 };
  }
  ...
  if (n < 3) {
    return { tau: 0, pValue: 1, significant: false, direction: "none", n };
  }

  // Kendall's tau between time indices and velocities
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      const indexDiff = indices[j] - indices[i]; // always positive
      const velDiff = velocities[j] - velocities[i];
      if (indexDiff * velDiff > 0) concordant++;
      else if (indexDiff * velDiff < 0) discordant++;
      // ties are excluded
    }
  }
  ...
  // Two-sided p-value, then convert to one-sided for deceleration test
  const twoSidedP = 2 * (1 - normalCDF(Math.abs(z)));

  // For deceleration test: we want to know if tau is significantly negative
  // p-value for H1: tau < 0 (deceleration)
  const decelPValue = tau < 0 ? twoSidedP / 2 : 1 - twoSidedP / 2;

  return {
    tau,
    pValue: decelPValue,
    significant: decelPValue < 0.05,
    direction: tau < -0.1 ? "decelerating" : tau > 0.1 ? "accelerating" : "none",
    n,
  };
}
